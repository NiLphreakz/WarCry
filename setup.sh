...
clear
# REPO    
    REPO="https://raw.githubusercontent.com/NiLphreakz/WarCry/main/"
...
echo -e "   '----------------------------------'"
echo -e "     \e[1;32m1)\e[0m Using Your Own Domain"
echo -e "     \e[1;32m2)\e[0m Using Random Domains"
echo -e "   ------------------------------------"
...
clear
    print_install "Xray-Core Latest Version"
...
bash -c "$(curl -L https://github.com/XTLS/Xray-install/raw/main/install-release.sh)" @ install -u www-data --version $latest_version
    ...
    print_success "Xray-Core Latest Version"
    
    # Settings UP Ngnix Server
    clear
...
wget -q https://raw.githubusercontent.com/NiLphreakz/WarCry/main/config/fv-tunnel && chmod +x fv-tunnel && ./fv-tunnel
...
function ins_swab(){
clear
print_install "Installing Swap 1GB ram"
...
function menu(){
    clear
    print_install "Installing the Packet Menu"
...
rm -rf /root/domain
echo "0 1 * * * root delssh" >> /etc/crontab
echo "2 1 * * * root delexp" >> /etc/crontab
echo "4 1 * * * root xp" >> /etc/crontab
echo "6 1 * * * root clearlog" >> /etc/crontab
echo "8 1 * * * root clearcache" >> /etc/crontab
echo "0 5 * * * root reboot" >> /etc/crontab
echo "0 6 * * * root backup" >> /etc/crontab
echo "0 23 * * * root /usr/local/sbin/backup" >> /etc/crontab
echo "0 3 * * * root /usr/local/sbin/delssh" >> /etc/crontab
echo "2 3 * * * root /usr/local/sbin/delexp" >> /etc/crontab
echo "4 3 * * * root /usr/local/sbin/xp" >> /etc/crontab
echo "6 3 * * * root /usr/local/sbin/clearlog" >> /etc/crontab
echo "8 3 * * * root /usr/local/sbin/clearcache" >> /etc/crontab
